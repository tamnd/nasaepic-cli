...
describes what users get. Pick a format with `-o`, or let nasaepic choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
nasaepic <command> -o table   # aligned columns for reading
nasaepic <command> -o jsonl   # one JSON object per line, for piping
nasaepic <command> -o json    # a single JSON array
nasaepic <command> -o csv     # spreadsheet friendly
nasaepic <command> -o tsv     # tab-separated
nasaepic <command> -o url     # just the URL column
nasaepic <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
nasaepic <command> --fields id,title,url
```
...
```bash
nasaepic <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
nasaepic <command>            # a table, because this is a terminal
nasaepic <command> | wc -l    # JSONL, because this is a pipe
```
